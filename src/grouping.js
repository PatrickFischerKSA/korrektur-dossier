export const roles={errors:'Fehlerliste',text:'Text mit Randbemerkungen',comments:'Kommentar'};
export function nameKey(value){return String(value).normalize('NFC').toLocaleLowerCase('de').replace(/ä/g,'ae').replace(/ö/g,'oe').replace(/ü/g,'ue').replace(/ß/g,'ss').normalize('NFKD').replace(/\p{M}/gu,'').replace(/[^\p{L}\p{N}]+/gu,' ').trim()}
const rules=[
 // The correction workshop exports its assessment using the original text's stem.
 // Consume the entire compound before considering the generic text markers.
 ['comments',/\b(?:mit[\s_-]+)?randbemerkungen[\s_-]+(?:korrektur|gutachten|beurteilung|bewertung|feedback|kommentar)\b/gi],
 ['text',/\b(?:(?:korrigiert(?:er|e|es|en)?[\s_-]+)?text[\s_-]+(?:mit[\s_-]+)?(?:randbemerkungen|randkommentaren?|randkommentierung|kommentaren?)|korrigiert(?:er|e|es|en)?[\s_-]+text|text[\s_-]+korrigiert|(?:mit[\s_-]+)?(?:randbemerkungen|randkommentare|randkommentierung))\b/gi],
 ['errors',/\b(?:fehlerliste|fehlerlisten|fehleruebersicht|fehlerprotokoll|fehleranalyse|fehlerverzeichnis|fehlerubersicht|fehler[\s_-]+liste)\b/gi],
 ['comments',/\b(?:gesamtkommentar|schlusskommentar|kommentar|kommentare|feedback|rueckmeldung|beurteilung|gutachten|bewertung|korrekturbericht|beurteilungsbericht)\b/gi],
 ['text',/\b(?:text|aufsatz|korrigiert(?:er|e|es|en)?)\b/gi],
];
export function identifyFilename(filename){
 filename=String(filename).normalize('NFC');
 const stem=filename.replace(/\.[^.]+$/,'').replace(/(?:\s*\(\d+\)|[\s_-]+(?:kopie|copy)(?:[\s_-]+\d+)?)$/i,'').replace(/([a-zäöüß])([A-ZÄÖÜ])/g,'$1 $2');
 // Keep offsets stable when matching umlauts by replacing them with one-character equivalents.
 let work=stem.toLowerCase().replace(/ä/g,'a').replace(/ö/g,'o').replace(/ü/g,'u').replace(/[_.,()[\]{}–—-]+/g,' ');
 // Match both transliterated and original spellings while retaining the original display name.
 const matches=[];
 // Teacher initials are metadata, including CamelCase variants such as korrFiP.
 // Restrict this to a suffix (optionally before an export role/version).
 work=work.replace(/\bkorr(?!ektur\b|igiert)\s*(?:[a-z]{2,5}|[a-z]{1,4}\s+[a-z])\b(?=\s*(?:(?:korrektur|kommentar|gutachten|fehlerliste)\s*)?(?:v\s*\d+|final)?\s*$)/g,(match,offset)=>{matches.push({start:offset,end:offset+match.length});return ' '.repeat(match.length)});

 work=work.replace(/\b(?:provisorisch(?:es|er|e)?|vorlaufig(?:es|er|e)?|uberarbeitet(?:es|er|e)?)\s+(?=(?:gutachten|kommentar|bewertung|beurteilung|text)\b)/g,(match,offset)=>{matches.push({start:offset,end:offset+match.length});return ' '.repeat(match.length)});
 for(const [kind,re]of rules){const pattern=new RegExp(re.source.replace('fehleruebersicht','fehler(?:ue|u)bersicht').replace('rueckmeldung','r(?:ue|u)ckmeldung'),re.flags);work=work.replace(pattern,(match,offset)=>{matches.push({kind,start:offset,end:offset+match.length});return ' '.repeat(match.length)})}
 const kinds=[...new Set(matches.filter(m=>m.kind).map(m=>m.kind))];
 let name=stem.split('').map((c,i)=>matches.some(m=>i>=m.start&&i<m.end)?' ':c).join('');
 name=name.replace(/(?:^|[\s_.-])(?:v(?:ersion)?\s*\d+(?:\.\d+)*|final|entwurf|korrigiert)(?=$|[\s_.-])/gi,' ').replace(/[_.,()[\]{}–—-]+/g,' ').replace(/\s+/g,' ').trim();
 const kind=kinds.length===1?kinds[0]:null;
 const prefix=stem.match(/^([^\s_.,–—-]+)[\s_.,–—-]+/)?.[1]||'';
 const group=prefix&&/^(?:[\p{L}]*\d[\p{L}\d-]*|[A-Z]{2,8}|Klasse[\p{L}\d-]*)$/u.test(prefix)&&nameKey(name).startsWith(nameKey(prefix)+' ')?prefix:'';
 let reason=kinds.length>1?'Mehrere Dokumentarten im Dateinamen.':!kind?'Dokumentart im Dateinamen nicht erkannt.':'';
 // Unknown suffixes must not silently become part of a pupil's name.
 const firstRole=Math.min(...matches.filter(m=>m.kind).map(m=>m.start));
 if(group&&Number.isFinite(firstRole)&&firstRole>prefix.length){
  const remainingTail=stem.split('').map((c,i)=>i<firstRole||matches.some(m=>i>=m.start&&i<m.end)?' ':c).join('').replace(/(?:v(?:ersion)?\s*\d+(?:\.\d+)*|final|entwurf)/gi,'').replace(/[_.,()[\]{}–—\s-]+/g,'').trim();
  if(remainingTail){reason=reason||'Unbekannter Zusatz im Dateinamen. Bitte Zuordnung bestätigen.';name=stem.slice(0,firstRole).replace(/[_.,()[\]{}–—-]+/g,' ').replace(/\s+/g,' ').trim();}
 }
 const key=nameKey(name);
 if(group)name=name.slice(prefix.length).trim();
 return {key,name,group,kind,reason:reason||(!key||!name?'Gemeinsamer Name im Dateinamen fehlt.':'')};
}
function comparisonKey(value){const parts=nameKey(value).split(' ');const first=parts[0];return /^[a-z]*\d[a-z\d]*$/i.test(first)?first+'|'+parts.slice(1).join(''):parts.join('')}
export function matchingDossiers(dossiers,key){return dossiers.filter(d=>comparisonKey(d.matchKey||[d.group,d.person||d.title].filter(Boolean).join(' '))===comparisonKey(key))}
export function completeness(d){const missing=[],duplicates=[];for(const [kind,label]of Object.entries(roles)){const n=d.sources.filter(s=>s.kind===kind).length;if(!n)missing.push(label);if(n>1)duplicates.push(label)}return {complete:!missing.length&&!duplicates.length,missing,duplicates,label:duplicates.length?'Doppelt: '+duplicates.join(', '):missing.length?'Fehlt: '+missing.join(', '):'Vollständig · 3 Dateien'};}
