import './style.css';
import {convertInBrowser} from './browser-word.js';
...
function renderWordSetup(){return `<details class="help"><summary>Hinweise zur Word-Ausgabe</summary><p>Word-Dateien werden automatisch hier im Browser verarbeitet. Tabellen, Farben und Formatierung werden übernommen; Schriftarten und Seitenumbrüche können von Word abweichen. Kommentare stehen vollständig in einem zusätzlichen Rand oder im gekennzeichneten Anhang. Vorhandene PDFs behalten ihre Originalseiten.</p><label class="check"><input id="native-layout" type="checkbox" ${nativeLayout?'checked':''}>Word-Formatierung übernehmen</label></details>`}
...
async function addFiles(list){
 ...
 try{
  for(const f of files){
   ...
   try{
    const parsed=nativeLayout&&/\.docx$/i.test(f.name)?await convertInBrowser(f):await importFile(f);if(!parsed.text?.trim()&&!parsed.original)throw Error('Kein lesbarer Text gefunden.');
