...
<div class="layout"><aside><p class="eyebrow">Deine Dossiers</p><div class="dossier-list">${dossiers.map(x=>`<button class="dossier-item ${x.id===active?'active':''}" data-select="${x.id}" ${busy?'disabled':''}>${esc(dossierLabel(x))}<small>${esc(completeness(x).label)}</small></button>`).join('')}</div><div class="aside-actions"><button id="new" ${busy?'disabled':''}>＋ Neues Dossier</button><button id="save" ${busy?'disabled':''}>Projekt sichern</button><button id="open" ${busy?'disabled':''}>Projekt öffnen</button><button id="all" ${busy||!canExportAll()?'disabled':''}>Alle PDFs als ZIP</button></div><p class="aside-note">Deine Inhalte bleiben in diesem Tab. Sichere das Projekt, bevor du ihn schliesst.</p></aside>
...
function canExportAll(){const items=dossiers.filter(d=>d.sources.length);return !pending.length&&items.length>0&&items.every(d=>completeness(d).complete)}
