import './style.css';
import {convertWithWord,wordToken} from './word.js';
...
function bind(){
 document.querySelector('#native-layout').onchange=e=>{nativeLayout=e.target.checked;render()};
...
function renderWordSetup(){return `<section class="panel"><h2>Original-Word-Layout</h2><label class="check"><input id="native-layout" type="checkbox" ${nativeLayout?'checked':''}>Word-Dateien mit Microsoft Word als PDF ausgeben</label>${wordToken()?'<p>Lokale Word-Anbindung aktiv. Word konvertiert temporäre Kopien; Originaldateien bleiben unverändert. Während des Exports bitte nicht in Word arbeiten.</p>':'<p>Für den originalgetreuen Export wird Word auf deinem Mac benötigt. Lade das lokale Paket herunter, entpacke es und öffne <strong>local/Dossier-starten.command</strong>. Voraussetzung: Microsoft Word und Node.js 24.</p><p style="margin-top:12px"><a href="./Dossier-Word-Mac.zip" download>Word-Ausgabe für Mac herunterladen</a> · <a href="http://127.0.0.1:5186/" target="_blank" rel="noopener">Bereits gestartete lokale Ausgabe öffnen</a></p>'}<p class="formats">${nativeLayout?'Word behält Textlayout, Tabellen und Umbrüche bei. Kommentare erhalten einen zusätzlichen Rand mit zitierten Textstellen; die Seiten werden dafür breiter. Keine überlagerten Seitenzahlen.':'Textmodus: Word-Layout wird neu gesetzt. Dies ist keine originalgetreue Ausgabe.'} Vorhandene Projekte mit nur extrahiertem Word-Text müssen für Original-Layout erneut aus den DOCX-Dateien eingelesen werden.</p></section>`}
...
async function addFiles(list){
 ...
 if(nativeLayout&&!wordToken()&&files.some(f=>/\.docx$/i.test(f.name))){notify('Bitte für Word-Dateien die lokale Word-Ausgabe starten. Hier wurde keine Datei eingelesen.');return;}
 ...
 try{
  for(const f of files){
   ...
   try{
    const parsed=nativeLayout&&/\.docx$/i.test(f.name)?await convertWithWord(f):await importFile(f);if(!parsed.text?.trim()&&!parsed.original)throw Error('Kein lesbarer Text gefunden.');
 ...
 try{
  ...
  if(all){const files={};
   for(let i=0;i<selection.length;i++){
    ...
    await new Promise(resolve=>setTimeout(resolve,0));
    try{files[`${String(i+1).padStart(2,'0')}-${safeName([d.group,d.person||d.title].filter(Boolean).join('-'))}.pdf`]=await createPdf(d,{nativeLayout});exportState.success++;}
  ...
  }else{const d=selection[0];download(await createPdf(d,{nativeLayout}),`${safeName([d.group,d.person,d.title].filter(Boolean).join(' - '))}.pdf`,'application/pdf');}
