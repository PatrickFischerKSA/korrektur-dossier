...
const browser=await chromium.launch({channel:'chrome'}),page=await browser.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto(process.env.APP_URL||'http://127.0.0.1:5178/');await page.getByText('Hinweise zur Word-Ausgabe',{exact:true}).click();await page.locator('#native-layout').uncheck();
