...
const docx=Buffer.from(zipSync({'word/document.xml':strToU8('<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>'+('Beispieltext mit Umlauten ä ö ü. '.repeat(300))+'</w:t></w:r></w:p></w:body></w:document>')}));
...
const browser=await chromium.launch({channel:'chrome'});const page=await browser.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto(process.env.APP_URL||'http://127.0.0.1:5178/');await page.locator('#native-layout').uncheck();
